		...
		<table id="cart-table">
			...
			<tbody>
				<c:forEach items="${carrinhoCompras.itens }" var="item">
					<tr>
					<!-- intes é uma instância de CarrinhoItem, q contém os produtos e os tipos -->
						...
						<td class="item-title">${item.produto.titulo }</td><!-- de item, vc puxa o produt, q puxa o título do produto -->
						<td class="numeric-cell">${item.preco }</td>
					
						<!-- carrinhoCompras pega os métodos get e set de CarrinhoItem... acho q através da ligação de hash.
						quando eu coloco item.fezes eu to pegando o método getFezes, assim como o item.preco pega o método getPreco
						de CarrinhoItem -->
						<td class="quantity-input-cell">
								<input type="text" min="0" id="quantidade" name="quantidade" value="${carrinhoCompras.getQuantidade(item,0) }" />
						</td>
						
						
						<td class="numeric-cell">${carrinhoCompras.getTotal(item,0) }</td>
